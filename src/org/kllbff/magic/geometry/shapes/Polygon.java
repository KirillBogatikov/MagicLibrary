...
public class Polygon extends Shape {
    ...
    public List<Shape> getIntersectionAreas(Shape other) {
        ArrayList<Point> verticesA = new ArrayList<>();
        ArrayList<Point> verticesB = new ArrayList<>();
        ArrayList<Point> vertices = new ArrayList<>();
        
        for(Point vx : other.getVertices()) {
            if(getPointPosition(vx) == PointPosition.OUTSIDE) {
                verticesA.add(vx);
            }
        }
        for(Point vx : getVertices()) {
            System.out.println(vx + " - " + other.getPointPosition(vx));
            if(other.getPointPosition(vx) != PointPosition.OUTSIDE) {
                verticesB.add(vx);
            }
        }
        
        for(Point vxA : verticesA) {
            LineSegment[] edgesA = other.getVertexEdges(vxA);
            for(Point vxB : verticesB) {
                LineSegment[] edgesB = getVertexEdges(vxB);
                Point in = edgesA[0].getIntersection(edgesB[0]);
                if(in != null) {
                    vertices.add(in);
                }
                in = edgesA[0].getIntersection(edgesB[1]);
                if(in != null) {
                    vertices.add(in);
                }
                in = edgesA[1].getIntersection(edgesB[0]);
                if(in != null) {
                    vertices.add(in);
                }
                in = edgesA[1].getIntersection(edgesB[1]);
                if(in != null) {
                    vertices.add(in);
                }
            }
        ...
        for(int i = vertices.size() - 1; i > -1; i--) {
            Point vx = vertices.get(i);
            if(vertices.indexOf(vx) < i) {
                vertices.remove(i);
                i++;
            }
        }
