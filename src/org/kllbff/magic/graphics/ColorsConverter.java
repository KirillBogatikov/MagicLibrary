...
public final class ColorsConverter {
    ...
    public static int toRGB(long color) {
        ColorSpace colorSpace = Color.colorSpace(color);

        switch(colorSpace) {
            case sRGB: return (Color.red(color) << 24) | (Color.green(color) << 16) | Color.blue(color);
            case XYZ: return toRGB(convertXYZtoRGB(color)); 
            case CIELAB: return toRGB(convertCIELABtoRGB(color));
            case HSB: return toRGB(convertHSBtoRGB(color));
